import { ObjectMapper, Schema } from './ObjectMapperLogic'
...
function objectMapperSchema2Shape(schema: Schema, label: string = 'data', path: string = '') {
  let result = null
  ...
  switch (schema._type) {
    case 'Object': {
      ...
      Object.keys(schema).forEach(key => {
        if (key[0] !== '_') {
          // @ts-ignore
          items.push(objectMapperSchema2Shape(schema[key] as Schema, key, path + label + '.'))
        }
      })
      result = {
        label : `${label}`,
...
const transformJSONShape = (obj: any, path: string = '') => {
  if (obj && typeof obj === 'object') {
    const isArray = Array.isArray(obj)

    return Object.keys(obj).map((key) => {
      const _key = isArray ? `[${key}]` : key
      const label: string = createKeyValueString(key, obj[key])
      const items: any = transformJSONShape(obj[key], path + _key + '.')
